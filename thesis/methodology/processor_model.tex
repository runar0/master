...
We will use these configurations to evalute how cache partitioning algorithms running on the third cache level responds to changes in size of private caches.
...
For 4-, 8- and 16-core simulations we will respectivly use a 4M, 8M and 16M L3 configuration.

Snipers memory model very limited compared to models in cycle-accurate simulators, such as gem5.
In our processor model each memory request completes in a constant 100ns, there is no simulation of row and colum access delays.
Because inter-core interractions may be out-of-order the memory bus model is a statistical model that attempts to estimate queuing delays based on a history window.
...
This implies that all simulated cores synchronize every 100 cycles.
An experiment will be conducted to explore the effect on our results when the width of this barrier is changed.
