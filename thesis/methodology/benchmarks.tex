...
Because simulating an entire benchmark is a time-consuming processe, we choose to extract a sample interval used to represent each benchmark.
...
They both work in a simular manner, by dividing the instruction stream of a benchmark into equal sized intervals. 
...
J. Yi et al.\todo{add cite} has shown that there is variation in accuracy between SimPoint and SimFlex selected intervals.
Because SimPoint integrates well with our selected simulator, Sniper, we chose to use SimPoint to extract intervals.

Ideally a few sample intervals are needed per benchmark in order to get accurate simulation results.
To simplify simulation, we however chose to extract one larger interval per benchmark, in place of multiple smaller ones.
