...
\section{Bechmarks and Workloads}
...
They both work in a similar manner, by dividing the instruction stream of a benchmark into equal sized intervals. 
Intervals are then classified, and a set of them chosen to represent the entire benchmark. 
What separates the two methods is the classification process used.
J. Yi et al.~\cite{Yi2005} has shown that there is variation in accuracy between SimPoint and SimFlex selected intervals.
As a result, We chose to use SimPoint because it integrates well with our selected simulator Sniper.

Ideally a few sample intervals are needed per benchmark in order to get accurate simulation results.
...
Because we are investigating performance changes due to various architectural choices, we are not that interested in correctly estimating the performance of the benchmark.
We are rather observing the correct change, which we believe to be possible even with only a single interval.
...
\todo{Results? (This and the next subsection)}
