...
Least recently used (LRU) replacement is one of the dominant cache replacement algorithms in hardware today. \todo{ref!}
...
In shared caches, LRU will favor access frequency, giving cores that issue many cache requests more cache space then those how issue few requests. 
...
Figure~\ref{fig:background:algorithms:rf_pattern} illustrates an example memory access pattern to a single cache set that is said to be recency-friendly.
...
\end{equation}
\caption{Streaming memory access  pattern (k = \todo{inf}) }
...
Most accesses would be streaming as the program iterates over the dataset, but some accesses will exhibit recency-friendly behavior as the program storing temporary results to memory during the iteration.
