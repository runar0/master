...
NUCache does not partition the cache by examining each cores access pattern seperatly like many other algorithms. 
...
The DeliTrack is a storage structure, indexed by PC that stores a miss count, insetion time and a next use histogram. 
...
Because if this implementation the MainWays may use the entire cache, if no DeliWays are in use, at the same time the DeliWays cannot exceed $M$.
