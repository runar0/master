...
While this is enough to implement simple schemes, such as \gls{lru} and \gls{tadip}, we need more information to implement cache partitioning schemes such as \gls{ucp}.
...
Of the schemes presented in Chapter~\ref{cpt:algorithms}, five have been implemented and tested in our simulation framework; \gls{lru}, \gls{tadip-f}, \gls{drrip}-3, \gls{prism}, \gls{ucp}, and \gls{pipp}.
...
\gls{lru}, which is included in Sniper, and the two set dueling schemes \gls{tadip} and \gls{drrip}, are all implemented as direct subclasses of the CacheSet class.
Based on the CacheSet class we have also implemented a cache with support for \glspl{atd}, also known as shadow tags.
By default, the \glspl{atd} do full sampling of the cache, but we made this configurable.
\gls{prism}, which requires access statistics for each core, is implemented on top of the \gls{atd} implementation.
Also, we create a \gls{umon} implementation on top of the \glspl{atd}; this adds the utility calculating and block assigning functionality needed by both \gls{ucp} and \gls{pipp}.
\gls{ucp} and \gls{pipp}, are then implemented on top of the \gls{umon} base class.
