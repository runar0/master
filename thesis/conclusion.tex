...
Figure~\ref{fig:conclusion:yearcomp} is a highly simplified view of 4-core random workload results, showing average \gls{stp} and \gls{hms} for all algorithms by publication year, normalized to \gls{lru} performance.
...
\gls{pipp} has shown comparable performance to \gls{lru} in 4-core random workloads, but has been unable to compete with \gls{lru} in most other cases.
...
In terms of misses, we have observed that \gls{drrip}, \gls{tadip}, and \gls{prism} have succeed in reducing the number of misses compared to \gls{lru}.
...
Independent of the performance metric used, none of the implemented algorithms has proven to beat \gls{ucp} on average.
